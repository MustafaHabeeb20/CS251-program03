  ...
  private: 
    struct bst_node {
      ...
      bst_node *right;
        ...
        //NEED TO ADD TO THE STRUCT                                                      NEED TO EDIT : INSERT, DELETE, FROM_VECTOR
    ...
    static  bst_node* buildbalancedtree(std::vector<bst_node*> &vectorA, int start, int end){
        if (start > end) {
            return nullptr;
                                                    ...
                                                    //EXAMPLE: the values of the unblances BST are put into a vector inorder
                                                    //         BSTnodevector:  < 7, 8, 9, 10, 11, 12, 13, 15 >
       
        int mid = (start + end) / 2;                //  start = 0 (position) --> value is 7
                                                    //  end = size() - 1 = 8 (there are 8 values in the vector) - 1 (for indexing) = 7 (for 7 position so 0 - 7)
                                                    // (start + end)/2 = (0+7)/2 = 3.5 = 3 (cause of ints)--> POSITION 3 IS THE MID WHICH CONTAINS A VALUE OF 10.
...
//**********************************************************************************************************************************************************************
  ...
  public:
   ...
   /* //find size of subtrees*******************************************************************************
    int subtreeSize(bst_node *p){ // root->left || root->right
        return _size(p);
    }
    */
...
private:
    ...
    void to_vectorHELPER(bst_node *p, std::vector<T> &vecB){
        if (p == nullptr){
            return;
        }
        //in-order into vector
        to_vectorHELPER(p->left,vecB);   //Left
        vecB.push_back(p->val);          //Print, in this case pushbackinto the vector
        to_vectorHELPER(p->right,vecB);  //Right
    }
public:
    std::vector<T> * to_vector() {
        std::vector<T> *inorderVector = new std::vector<T>;
        
        to_vectorHELPER(root, *inorderVector);
    ...
    bool get_ith(int i, T &x) {
        ...
        x = check_smallest(i, root);
        return true;
    }
    
private:
    //helper function to find the val for the ith position
    int check_smallest(int checki, bst_node *p){
        ...
        //if checki is less than the countLeftnodes, move position to the left of the tree to narrow down on the search.
            ...
            return check_smallest(checki, p->left);
        }
            ...
            return check_smallest(checki - (p->countLeftnodes + 1), p->right);
        }
    }
...
private:
  ...
  public:
    ...
    int position_of(const T & x) {
        ...
        x = find_the_position(x, root);
        return x;
    }
    
private:
    //find the position of x recursivly.
    int find_the_position(const T & lookingforVal, bst_node *p){
        int findpos;
        
        if(lookingforVal == p->val){
            findpos = (p->countLeftnodes) + 1;
            return findpos;
        }
        
        else if(lookingforVal < p->val){
            return find_the_position(lookingforVal, p->left);
        }
        
        else if (lookingforVal > p->val){
            findpos = (p->countLeftnodes) + 1;
            return find_the_position(lookingforVal, p->right);
        }
        else{
            //if not found
            return -1;
...
public:
    ...
    int num_geq(const T & x) {
        ...
        int countergreat = 0;
        numgeqHelper(x, &countergreat, root);
        
      return countergreat;  // placeholder
    }
    
private:
    void numgeqHelper(const T & greaterVal, int *counterA, bst_node *p){
        //base case
        if(p == nullptr){
            return;
        }
        ...
        if(p->val == greaterVal){
            (*counterA)++;
            return numgeqHelper(greaterVal, counterA, p->right);
        }
        ...
        // if the current node val is greater than the val we are comparing...
            ...
            return numgeqHelper(greaterVal, counterA, p->left);
        }
  ...
  public:
    ...
    int num_leq(const T &x) {
        int counterless = 0;
        numleqHELPER(x, &counterless, root);   //edit
        
      return counterless;  // placeholder
    }
    
private:
    //
    void numleqHELPER(const T & lesserVal, int *counterB, bst_node *p){
        //base case
        if(p == nullptr){
            return;
        }
        ...
        if(p->val == lesserVal){
            (*counterB)++;
            return numleqHELPER(lesserVal, counterB, p->left);
        }
        ...
        // if the current node val is less than the val we are comparing...
            ...
            return numleqHELPER(lesserVal, counterB, p->right);
        }
...
public:
  ...
  public:
    ...
    int num_range(const T & min, const T & max) {
        ...
        whatstheRange = numrangeHELPER(min, max, root);
        
      return whatstheRange;
    }
    
    
private:
 
    int numrangeHELPER(const T & low, const T & high, bst_node *p){
  ...
  public:
    ...
    std::vector<T> * extract_range(const T & min, const T & max) {
        std::vector<T> ERvector = new std::vector<T>;
        
        return extractrangeHELPER(root, min, max, ERvector);
    }
private:
    // Returns vector in range [low, high]
    std::vector<T> * extractrangeHELPER(bst_node *p, const T & low, const T & high, std::vector<T> vectorD){
      ...
      /*  //want to first get the root to the min val.
        if (p->val == low){
            
            //if its at the min val or greater than, it should pushback the val to the vector and move to the right to go inorder
            if (p->val >= low){
                vectorD.push_back(p->val);
                extractrangeHELPER(p->right, low, high, vectorD);
            }
            
            //if its at the max val or less than, it should pushback the val to the vector and move to the left to go inorder
            else if (p->val <= high){
                vectorD.push_back(p->val);
                extractrangeHELPER(p->left, low, high, vectorD);
            }
            
            //if its not inbetween the range, return the vector
            else{
                return vectorD;
            }
        }
        //if not at the low, get to the low to start pushing back inorder.
        else{
            if(p->val > low){
                extractrangeHELPER(p->left, low, high, vectorD);
            }
